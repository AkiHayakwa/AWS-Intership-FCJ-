...
* **Thông tin ghi nhận**: Định danh chi tiết của IAM user/role, các hoạt động API (`GetObject`, `PutObject`, `DeleteObject`), ngữ cảnh xác thực (MFA, chuỗi vai trò được đảm nhận) và truy cập chéo tài khoản.
...
1. **Tạo CloudTrail Trail**: Trong bảng điều khiển CloudTrail, tạo một trail mới (ví dụ: `s3-data-events-trail`) và hướng đích về một S3 bucket trung tâm.
...
*Lưu ý: Đối với Organization trail, bổ sung cột phân vùng `organization_id STRING` đầu tiên và chèn nó vào cấu trúc `storage.location.template`.*

---

## Các Mẫu Truy Vấn Bảo Mật Chính

Bảng Athena sử dụng partition projection yêu cầu bạn **phải** chỉ định phân vùng `account`, `region` và `timestamp` trong mệnh đề `WHERE` để thu hẹp phạm vi quét dữ liệu, tránh quét toàn bộ bảng gây phát sinh chi phí lớn.
...
Liệt kê các thao tác gặp lỗi phân quyền (`AccessDenied`, `NoSuchKey`), giúp nhận diện hành vi dò quét tài nguyên trái phép:
...
Theo dõi các thao tác xóa trong một khoảng ngày, ghi nhận vai trò đảm nhận và trạng thái xác thực MFA:
...
* **Ưu Tiên Lọc Phân Vùng**: Luôn lọc theo cột phân vùng (`account`, `region`, `timestamp`) trước. Việc chỉ dùng `eventtime` để lọc thời gian sẽ quét toàn bộ dữ liệu, tăng chi phí.
* **Loại Trêu HeadObject**: Nếu chi phí tăng cao do các tác vụ siêu dữ liệu, hãy cấu hình CloudTrail advanced event selectors để loại trừ cuộc gọi API `HeadObject`.
* **Tính Toàn Vẹn và Bảo Mật**: Kích hoạt tính năng log file validation để ngăn can thiệp tệp log. Nên lưu trữ nhật ký tại một tài khoản AWS chuyên biệt (Log Archive account).
* **Lỗi HIVE_PARTITION_SCHEMA_MISMATCH**: Khi gặp lỗi này, hãy đối chiếu `storage.location.template` xem đã khớp hoàn toàn với đường dẫn thực tế trên S3 chưa.
